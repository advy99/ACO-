...
void run_interface(int argc, char ** argv) {
	...
	ACOppInterface interface("Prueba SDL", 100, 100, WIDTH, HEIGHT, 0);
	...
	for (uint32_t i = 0; i < best_path.size() - 1; i++) {
		std::unique_ptr<Line> line = std::make_unique<Line>(points[best_path[i]].x, points[best_path[i]].y,
																			 points[best_path[i + 1]].x, points[best_path[i + 1]].y, LINE_WIDTH, Color(0, 0, 200, 255));
		interface.add_object(std::move(line));
	}
	interface.add_object(std::make_unique<Line>(points[best_path.front()].x, points[best_path.front()].y,
